...
int unpack(int packed_input) { return ntohs(packed_input); }
...
int get_and_bind_socket() {
  ...
  // We end up with a linked-list of address, and we want to onenct to the first
  // one we can
  for (p = server_info; p != NULL; p = p->ai_next) {
    ...
    if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
      // If something went wrong binding this socket, we can close it and move
      // on to the next one.
      close(sockfd);
      perror("server: bind");
      continue;
    ...
    // If we've made it this far, we ahve a valid socket and can stop iteratingn
    // through.
    break;
  }
...
int main(void) {
  ...
  while (1) {
    ...
    // Once we've accepted an incoming request, we can read from it into a
    // buffer.
    int buffer;
