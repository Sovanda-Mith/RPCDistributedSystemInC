...
int get_socket() {
  ...
  for (p = server_info; p != NULL; p = p->ai_next) {
    ...
    if ((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
      // Something went wrong getting this socet, so we can try the next one.
    ...
    // If we've made it this far, we have a valid socket and can sttop iterating
    // through.
    break;
  }
...
bool is_prime_rpc(int number) {
  // First, we need to pack the data, ensuring that it's sent across the network
  // in the right format
  ...
  int bytes_receive = recv(sockfd, &buf, 1, 0);
  if (bytes_receive == -1) {
    perror("recv");
    exit(1);
  }
  ...
  // All done! Closethe socket and return the result.
  close(sockfd);
  return result;
}
